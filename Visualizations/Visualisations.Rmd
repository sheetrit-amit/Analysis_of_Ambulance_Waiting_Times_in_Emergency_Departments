...
# Create larger bins for Time_diff
df <- df %>%
  mutate(Time_diff_bin = cut(Time_diff, breaks = seq(0, max(Time_diff), by = 5), include.lowest = TRUE))

# Calculate the average Percentage_Arrived for each bin
data_grouped <- df %>%
  group_by(Time_diff_bin) %>%
  summarise(Avg_Percentage_Arrived = mean(Percentage_Arrived, na.rm = TRUE))

# Generate the bar plot
ggplot(data_grouped, aes(x = Time_diff_bin, y = Avg_Percentage_Arrived)) +
  geom_bar(stat = "identity", fill = "blue", alpha = 0.7, color = "black") +
  labs(title = "Bar Plot of Binned Time_diff with Average Percentage_Arrived",
       x = "Time_diff (binned)",
       y = "Average Percentage_Arrived") +
  theme_minimal() +
